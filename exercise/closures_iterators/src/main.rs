...
fn main() {
    ...
    let rewrite = vec![(0, 1), (2, 3), (4, 5)];
    let mut transformed : Vec<(i32, i32)> = Vec::new();
    for var in rewrite {
        let tmp : (i32, i32) = (var.0 + 1 , var.1 +1); 
    ...
    numbers
        .iter_mut()
        .map(|x| *x*3)
        .for_each(|x| println!("Here is the number multipled {:?}", x))
